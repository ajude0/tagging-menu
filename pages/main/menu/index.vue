<template>
  ...
  <div class="flex justify-end">
    <button
      v-if="hasPermission('Menu', 'can_add') || role == 'Admin'"
      @click="showModal2 = true"
      class="py-3 px-4 bg-green-700 rounded-md mb-4 text-white flex gap-1"
    >
      <svg
          ...
          <table class="table-auto min-w-full rounded-xl">
            <thead>
              <tr class="bg-gray-50 gap-3">
                ...
                <th
                  v-if="
                    hasPermission('Menu', 'can_delete') ||
                    role == 'Admin' ||
                    hasPermission('Menu', 'can_edit')
                  "
                  scope="col"
                  class="p-5 text-left whitespace-nowrap text-sm leading-6 font-semibold text-gray-900 capitalize"
                >
            ...
            <tbody class="divide-y divide-gray-300">
              ...
              >
                ...
                <td class="flex p-5 items-center gap-0.5">
                  <button
                    v-if="hasPermission('Menu', 'can_edit') || role == 'Admin'"
                    @click="openEditForm(menu.id)"
                  ...
                  <button
                    @click="deleteMenu(menu.id, $swal)"
                    v-if="
                      hasPermission('Menu', 'can_delete') || role == 'Admin'
                    "
  ...
  >
    <div class="w-full max-w-xl bg-white shadow-lg rounded-lg p-6 relative">
      <div class="flex items-center pb-3 border-b border-gray-300">
        <h3 class="text-gray-800 text-xl font-bold flex-1">Create Menu</h3>
        <svg
      ...
      <div class="my-6">
        ...
        >
          ...
          <!-- Buttons inside the form for proper form submission -->
          <div
            class="sm:col-span-2 border-t border-gray-300 pt-6 flex justify-end gap-4"
          >
            <button
              type="button"
              @click="showModal2 = false"
            ...
            <button
              type="submit"
              class="mx-2 px-4 py-2 text-sm rounded text-white bg-blue-500 focus:outline-none hover:bg-blue-300"
            >
              Send
            </button>
          </div>
...
const openEditForm = async (menuId) => {
  getMenu(menuId);
  editModal.value = ref(true);
};
const editForm = async () => {
  editMenu(updateMenu.value.id, updateMenu.value.name, $swal);
  editModal.value = false;
};
const submitForm = async () => {
  createMenu(menuName.value, $swal);
